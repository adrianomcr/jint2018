    ...
    * End latitude
    ...
    for row_number, row in enumerate(reader):
        try:
            ...
            graph.add_edge(start_node, end_node, weight=length, id=row)
...
def chinese_postman_paths(graph, n=5):
    ...
    for cost, matching in matchings[:n]:
        ...
        print 'build_eulerian_graph time: ', time.time()-ccc
...
def create_graph(list_tuple):
    ...
    graph = nx.Graph()
    #for row_number, row in enumerate(reader):
    for e in range(len(list_tuple)):
        start_node = list_tuple[e][0]
        #print 'start_node: ', start_node
        end_node = list_tuple[e][1]
        #print 'end_node: ', end_node
        length = list_tuple[e][2]
        #print 'length: ', length
...
def main_CPP(list_tuple, start_node):


    graph = create_graph(list_tuple)
    ...
    for eulerian_graph, nodes in paths:
        ...
        ok = False
        count = 0
        while (not ok and count < len(nodes)):
            count = count + 1
            if (nodes[0] != start_node):
                move_node = nodes.pop(0)
...
if __name__ == '__main__':
    ...
    #print 'type(args.input): ', type(args.input)
